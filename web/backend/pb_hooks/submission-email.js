...
function notify(e) {
	if ($os.getenv("DASHBOARD_DISABLE_EMAIL") === "1") return
	...
	const oldStatus = record.getString("__previous_status")

	// 3. Post-update logic: Send email if status changed
	if (oldStatus && oldStatus !== newStatus) {
		logger.info("Submission status changed", "id", recordId, "old_status", oldStatus, "new_status", newStatus)
		...
		const bulkRejectId = record.getString("__bulk_reject_id")
...
module.exports = { notify, sendBulkRejectionEmails }
