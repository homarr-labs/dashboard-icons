...
Set `OG_REFRESH_COUNT=true pnpm generate:og` to use the public search catalog's current total across native and external collections. The default preview uses the approved 9,222 count; generating either version downloads the Hermes Agent icon from LobeHub. A failed or incomplete catalog response stops the refresh rather than publishing a misleading count.
