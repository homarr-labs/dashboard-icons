...
class Handler(BaseHTTPRequestHandler):
 ...
 def do_POST(self):
  ...
  if self.path=='/simulate':
   r=runs[str(body['id'])];r.update(status='completed',conclusion=body.get('conclusion','failure'))
   if body.get('commit_sha'): commits.insert(0,{'sha':body['commit_sha'],'commit':{'message':'Dashboard-Publish-Batch: '+r['display_title'].removeprefix('Publish ')}})
   self.reply(r);return
 ...
 def do_GET(self):
  ...
  if '/commits' in self.path:self.reply(commits);return
